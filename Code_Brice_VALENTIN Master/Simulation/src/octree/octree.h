...
public:
    ...
    std::vector<int> m_elems; // contained elements, if leaf
    Octree() : m_children(NULL) {}
    ~Octree() { delete[] m_children; }
    const TBBox& bbox() const { return m_bbox; }
    const std::vector<int>& elems() const { return m_elems; }
    bool isLeaf() const { return m_children==NULL; }
    void init(const TBBox& bbox, const std::vector<TBBox>& elemBB, const std::vector<int>& elems, int elemsPerLeaf = 8, int maxLevel = 6)
    {
        m_bbox = bbox;
    ...
    void init(const std::vector<TBBox>& elemBB, int elemsPerLeaf = 8, int maxLevel = 6)
    {
        TBBox bbox;
        bbox.clear();
    ...
    void findAllAround(std::vector<TOctree*>& cells, const Coord& p, Real dist)
    {
        TOctree* n = this;
        if (n->isLeaf())
        {
