...
RenderFlag render_flag[RENDER_NFLAGS] = {
    ...
    RenderFlag(0, 4, "display FEM particles"),
...
void initgl()
{
    ...
    {
        textureColor = loadTexture(simulation_params.colorMap.c_str());
        if (textureColor != 0)
        {
            render_meshes[0]->textureFilename = simulation_params.colorMap.c_str();
            render_meshes[0]->color = TColor(1,1,1,1);
        }
        if (use_shaders && textureColor != 0)
        {
            textureNormal = loadTexture(simulation_params.normalMap.c_str());
            if (textureNormal != 0)
            {
                render_meshes[0]->computeTangents = true;
                SurfaceMesh_updateNormals(render_meshes[0]);
            }
        }
    }
    shaders_reload();
...
void shaders_reload()
{
    ...
    else
    {
        ...
        shaderVertex = loadShader(GL_VERTEX_SHADER_ARB, simulation_params.shaderVertex);
        shaderFragment = loadShader(GL_FRAGMENT_SHADER_ARB, simulation_params.shaderFragment);
...
template<class T>
void myglVertexPointer(std::vector<T>& x)
{
    const GLvoid * pointer = &x[0];
    glVertexPointer(sofa::defaulttype::DataTypeInfo<T>::size(), myglType<T>(), sizeof(T), pointer);
}

template<class T>
void myglNormalPointer(std::vector<T>& x)
{
    const GLvoid * pointer = &x[0];
...
template<class T>
void myglColorPointer(std::vector<T>& x)
{
    const GLvoid * pointer = &x[0];
...
template<class T>
void myglTexCoordPointer(std::vector<T>& x)
{
    const GLvoid * pointer = &x[0];
...
template<class T>
void myglVertexAttribPointerARB(GLint attrib, std::vector<T>& x, bool normalized = false)
{
    const GLvoid * pointer = &x[0];
...
template<class T>
void myglDrawElements(GLenum mode, std::vector<T>& x)
{
    const GLvoid * pointer = &x[0];
...
void render()
{
    ...
    bool render_fem_points = (render_flag[RENDER_FLAG_POINT]) || (render_flag[RENDER_FLAG_MESH] == ((render_meshes.empty())? 1 : 2));
    TColor bgcolor;
    ...
    if (fem_mesh)
    {
        ...
        if (render_fem_points)
        {
            myglVertexPointer(mesh->positions);
            if (!render_surface)
            {
                glColor3f(0.1f,0.1f,0.1f);
                glPointSize(2*(render_flag[RENDER_FLAG_POINT] ? render_flag[RENDER_FLAG_POINT] : 1));
            ...
            if (!fem_mesh->fixedParticles.empty())
            {
                ...
                glPointSize(4*(render_flag[RENDER_FLAG_POINT] ? render_flag[RENDER_FLAG_POINT] : 1));
            ...
            for (unsigned i=0;i<simulation_params.fixedBBox.size();i++) {
                TVec3 p0 = TVec3(simulation_params.fixedBBox[i].min[0],simulation_params.fixedBBox[i].min[1],simulation_params.fixedBBox[i].min[2]);
                TVec3 p1 = TVec3(simulation_params.fixedBBox[i].min[0],simulation_params.fixedBBox[i].min[1],simulation_params.fixedBBox[i].max[2]);
                TVec3 p2 = TVec3(simulation_params.fixedBBox[i].min[0],simulation_params.fixedBBox[i].max[1],simulation_params.fixedBBox[i].min[2]);
                TVec3 p3 = TVec3(simulation_params.fixedBBox[i].min[0],simulation_params.fixedBBox[i].max[1],simulation_params.fixedBBox[i].max[2]);
                TVec3 p4 = TVec3(simulation_params.fixedBBox[i].max[0],simulation_params.fixedBBox[i].min[1],simulation_params.fixedBBox[i].min[2]);
                TVec3 p5 = TVec3(simulation_params.fixedBBox[i].max[0],simulation_params.fixedBBox[i].min[1],simulation_params.fixedBBox[i].max[2]);
                TVec3 p6 = TVec3(simulation_params.fixedBBox[i].max[0],simulation_params.fixedBBox[i].max[1],simulation_params.fixedBBox[i].min[2]);
                TVec3 p7 = TVec3(simulation_params.fixedBBox[i].max[0],simulation_params.fixedBBox[i].max[1],simulation_params.fixedBBox[i].max[2]);
        ...
        if (render_fem_tetra)
        {
            ...
            const TVec3* x = &mesh->positions[0];
            const TTetra* tetrahedra = &mesh->tetrahedra[0];
        ...
        if (render_flag[RENDER_FLAG_DEBUG]==1)
        {
            if (mesh->velocity.size() == mesh->positions.size())
            {
                ...
                {
                    TVec3 x = mesh->positions[i];
                    TVec3 x2 = x + mesh->velocity[i]*0.1;
            ...
            if (mesh->a.size() == mesh->positions.size())
            {
                ...
                for (unsigned int i=0;i<mesh->positions.size();++i)
                {
                    TVec3 x = mesh->positions[i] + mesh->velocity[i]*0.1;
                    TVec3 x2 = x + mesh->a[i]*0.1;
                    glVertex3fv(x.ptr()); glVertex3fv(x2.ptr());
                }
                glEnd(); // GL_LINES
            }
        ...
        if (mesh->externalForce.index != -1)
        {
            TVec3 p1 = mesh->positions[mesh->externalForce.index];
            TVec3 p2 = p1 + mesh->externalForce.value * 0.001;
            glLineWidth(3);
            glBegin(GL_LINES);
    ...
    if (render_flag[RENDER_FLAG_DEBUG]==1)
    {
        ...
        for (unsigned int mi=0;mi<render_meshes.size();++mi)
        {
            ...
            const TVec3* positions = &mesh->positions[0];
            const TVec3* normals = mesh->normals.empty() ? NULL : &mesh->normals[0];
            const TVec3* tangents = mesh->tangents.empty() ? NULL : &mesh->tangents[0];
            ...
            for (unsigned int i=0;i<mesh->positions.size();++i)
            {
                ...
                if (normals)
                {
                    TVec3 pn = p + normals[i] * (simulation_size*0.002);
                    glColor3f(0,0,1);
                    glVertex3fv(p.ptr());
                    glVertex3fv(pn.ptr());
                }
                if (tangents)
                {
                    TVec3 pt = p + tangents[i] * (simulation_size*0.002);
                    glColor3f(0,1,0);
                    glVertex3fv(p.ptr());
                    glVertex3fv(pt.ptr());
                }
