...
{
    //picked_position = TVec3(obj_x, obj_y, obj_z);
    picked_particle = -1;
    if (!fem_mesh) return -1;
    if (fixed_particle != -1)
    {
        FEMMesh_removeFixedParticle(fem_mesh,fixed_particle);
        fixed_particle = -1;
    }
    ...
    const TVec3* pos = &fem_mesh->positions[0];
...
int update_picking(int picking_mode)
{
    ...
    if (picking_mode != -1 && picked_particle != -1)
    {
        ...
        case 3: // middle click
            if (fixed_particle == -1)
            {
                if (!FEMMesh_isFixedParticle(fem_mesh,picked_particle))
                {
                    FEMMesh_addFixedParticle(fem_mesh,picked_particle);
...
void render_gui()
{
    ...
    if (render_flag[RENDER_FLAG_STAT])
    {
        ...
        drawTextFont(13);
        y0 += drawTextF("%s\n",simulation_params.device_name.c_str());
        ...
        if (simulation_params.subSteps == 1)
            y0 += drawTextF("Time Step: %g\n",simulation_params.timeStep);
        else
            y0 += drawTextF("Time Step: %g / %d\n",simulation_params.timeStep,simulation_params.subSteps);
